...
ezra({
  ...
  try {
    ...
    const data = response.data;
    const messageText = `
📖 ☢️TOXIX LOVER-V2 XMD HOLY BIBLE 📖
...
ezra({
  ...
  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 254717xxxxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Wait 🎁TOXIC LOVER-V2 XMD🎁 is getting your pair code ...*";
...
*❤️‍🔥TOXIC LOVER-V2 XMD❤️‍🔥ELEMENT  INFO:*
🚀 *Name:* ${data.name}
🚀 *Symbol:* ${data.symbol}
...
> Powered by ❤️‍🩹TOXIC LOVER-V2 XMD❤️‍🩹
    `;
    repondre(texa);
  } catch (err) {
    console.error(err);
